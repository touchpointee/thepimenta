...
export default function PackagesEditor() {
    ...
    const [packages, setPackages] = useState<{ title: string; duration: string; price: string; image: string }[]>([])
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            if (data?.content?.packages) {
                setPackages(data.content.packages)
            }
        } catch (error) {
    ...
    const addPackage = () => {
        setPackages([...packages, { title: '', duration: '', price: '', image: '' }])
    }
    ...
    return (
        <div>
            <div className="flex items-center justify-between mb-8">
                <h1 className="text-3xl font-bold text-gray-900">Edit Packages</h1>
                <button
                    onClick={addPackage}
                            ...
                            <div key={index} className="p-6 bg-gray-50 rounded-xl border border-gray-100 relative group">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="space-y-4">
                                        <div>
                                            <label className="block text-xs font-medium text-gray-500 mb-1">Package Title</label>
                                            <input
                                                type="text"
                                                value={pkg.title}
                                                onChange={(e) => updatePackage(index, 'title', e.target.value)}
                                                className="w-full px-3 py-2 rounded border border-gray-200 text-sm"
                                                placeholder="e.g., Kerala Cooking Holiday"
                                            />
                                        </div>
                                    ...
                                    <div>
                                        ...
                                        {pkg.image && (
                                            <div className="mt-2 relative h-32 w-full rounded-lg overflow-hidden bg-gray-200">
                                                <img src={pkg.image} alt="Preview" className="w-full h-full object-cover" />
                                            </div>
                                        )}
                                        {!pkg.image && (
